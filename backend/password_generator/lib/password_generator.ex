defmodule PasswordGenerator do
  ...
  @doc """
  Generates password fro given options:
...
@spec generate(options :: map()) :: {:ok, bitstring()} | {:error, bitstring()}
  ...
  defp validate_options_values_are_boolean(false, _length, _options) do
    {:error, "Only booleans allowed fro options values"}
  end
  ...
  defp validate_options_values_are_boolean(true, length, options) do
    ...
    valdate_options(invalid_options?, length, options)
  end

  defp valdate_options(true, _length, _options) do
    {:error , "Only options allowed numbers, uppercase, symbols."}
  end

  defp valdate_options(false, length, options) do
  ...
  defp generate_strings(length, options) do
    ...
    random_strings = generate_ramdom_strings(length, options)
  ...
  defp generate_ramdom_strings(length, options) do
